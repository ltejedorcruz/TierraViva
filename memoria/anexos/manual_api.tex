...
La API se ejecuta en la Raspberry Pi y trabaja sobre los datos almacenados previamente en SQLite. Las lecturas siguen un flujo definido en SQLite. Las lecturas siguen un flujo definido: las estaciones las publican en ThingSpeak, el ingestor \texttt{main.py} las recupera, \texttt{database.py} las almacena y \texttt{api.py} las expone posteriormente mediante \textit{endpoints} HTTP.
...
\begin{itemize}
    ...
    \item \texttt{ok: false}: la petición es válida y la API devuelve una respuesta sin registros disponibles.
\end{itemize}

El parámetro \texttt{station} se normaliza a mayúsculas. Si se omite, la API utiliza la estación configurada por defecto mediante \texttt{STATION\_ID}. Si la estación consultada carece de registros almacenados, la API devuelve una respuesta equivalente a una consulta sin datos disponibles.
...
Las variables de ThingSpeak pertenecen al proceso de ingesta \texttt{main.py}, encargado de recuperar las lecturas desde la plataforma antes de que la API las consulte en SQLite.
...
Ante la ausencia del fichero, la API devuelve un error HTTP 404:

\begin{jsoncode}
{
  "detail": "No se encontro frontend/index.html"
}
\end{jsoncode}
...
El parámetro \texttt{station} se normaliza a mayúsculas. Si la estación consultada carece de registros almacenados, la API devuelve una respuesta equivalente a una consulta sin datos disponibles.
...
\hline
404 & Fichero de \textit{dashboard} ausente & Falta el fichero \texttt{frontend/index.html}. \\
\hline
422 & Parámetro inválido & Un parámetro queda fuera de las restricciones definidas, por ejemplo \texttt{limit=0}. \\
\hline
500 & Error interno & Fallo inesperado durante la ejecución. Deben revisarse los \textit{logs}. \\
\hline
\end{tabular}
...
La API de TierraViva proporciona una capa local de consulta sobre los datos almacenados en la Raspberry Pi. Sus \textit{endpoints} permiten obtener configuración, última lectura, histórico, estadísticas y estado básico del servicio, y sirven como punto de acceso para el \textit{dashboard} web de supervisión alojado en la propia Raspberry Pi. 