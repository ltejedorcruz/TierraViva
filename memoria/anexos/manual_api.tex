...
Este anexo documenta la API local de TierraViva implementada sobre FastAPI. Su finalidad es servir como referencia técnica para consultar los endpoints disponibles, sus parámetros, las respuestas esperadas y los errores más habituales.
...
\begin{itemize}
    \item servir el dashboard web desde la ruta raíz;
...
Todas las respuestas de la API se devuelven en formato JSON, salvo la ruta raíz \texttt{/}, que sirve el fichero HTML del dashboard.
...
\hline
\rowcolor{gray!20}
\textbf{Variable} & \textbf{Valor por defecto} & \textbf{Uso} \\
\hline
...
\hline
\texttt{WEB\_REFRESH\_SECONDS} & \texttt{3} & Intervalo de refresco recomendado para el dashboard. \\
\hline
\texttt{DATA\_STALE\_SECONDS} & \texttt{90} & Tiempo utilizado para clasificar una lectura como reciente, antigua u offline. \\
\hline
\end{tabular}
...
\hline
GET & \texttt{/} & Sirve el dashboard web desde \texttt{frontend/index.html}. \\
\hline
...
\hline
GET & \texttt{/api/health} & Comprueba el estado básico de API, base de datos y frontend. \\
\hline
\end{tabular}
\caption{Endpoints disponibles en la API actual de TierraViva.}
\label{tab:endpoints_api_actual}
\end{table}

\section{GET /}

Sirve la interfaz web principal del dashboard.
...
Si el fichero \texttt{frontend/index.html} existe, el navegador muestra el dashboard de TierraViva.
...
\hline
\rowcolor{gray!20}
\textbf{Parámetro} & \textbf{Tipo} & \textbf{Obligatorio} & \textbf{Descripción} \\
\hline
...
Las lecturas devueltas por la API proceden de la tabla \texttt{readings}. La Tabla \ref{tab:campos_lectura_api_resumido} resume los campos más relevantes para el consumo desde el dashboard.
...
\hline
\texttt{relay\_state} & entero & Estado del relé, si está disponible como campo normalizado. \\
\hline
\texttt{system\_event} & entero & Evento de sistema, si está disponible como campo normalizado. \\
\hline
\texttt{debug\_code} & entero & Código auxiliar de diagnóstico, si está disponible como campo normalizado. \\
\hline
...
El endpoint \texttt{/api/latest} añade dos clasificaciones calculadas por la API:
...
\hline
\rowcolor{gray!20}
\textbf{Endpoint} & \textbf{Parámetro} & \textbf{Validación} \\
\hline
...
\hline
\rowcolor{gray!20}
\textbf{Código} & \textbf{Situación} & \textbf{Interpretación} \\
\hline
200 & Petición correcta & La API ha procesado la petición. El campo \texttt{ok} indica si existen datos. \\
\hline
404 & Dashboard no encontrado & Falta el fichero \texttt{frontend/index.html}. \\
\hline
422 & Parámetro inválido & Un parámetro no cumple las restricciones, por ejemplo \texttt{limit=0}. \\
\hline
500 & Error interno & Fallo no controlado durante la ejecución. Deben revisarse los logs. \\
\hline
\end{tabular}
...
\section{Relación con el dashboard}

El dashboard consume los endpoints de la API para actualizar la interfaz:
...
\hline
\rowcolor{gray!20}
\textbf{Endpoint} & \textbf{Uso en dashboard} \\
\hline
...
\hline
\end{tabular}
\caption{Uso de los endpoints por parte del dashboard.}
\label{tab:relacion_api_dashboard_resumida}
\end{table}

\section{Conclusión}

La API de TierraViva proporciona una capa local de consulta sobre los datos almacenados en la Raspberry Pi. Sus endpoints permiten obtener configuración, última lectura, histórico, estadísticas y estado básico del servicio. 