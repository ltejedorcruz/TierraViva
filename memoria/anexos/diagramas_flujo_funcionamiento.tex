...
Este anexo resume los principales flujos de funcionamiento de TierraViva. Su finalidad es mostrar cómo se comporta el sistema durante una ejecución normal y ante situaciones de seguridad, complementando los anexos específicos dedicados a instalación, estructura de ThingSpeak y referencia de la API.
...
\begin{figure}[H]
    ...
    \caption{Flujo general de funcionamiento de TierraViva. Elaboración propia, véase Anexo~\ref{sec:uso_ia_generativa_tfg}.}
...
\begin{figure}[H]
    ...
    \includegraphics[width=\textwidth]{figs_png/ciclo_estacion_tierraviva.png}
        \caption{Ciclo de funcionamiento de una estación TierraViva. Elaboración propia, véase Anexo~\ref{sec:uso_ia_generativa_tfg}.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.80\textwidth]{figs_png/decision_local_riego_resumida.png}
        \caption{Flujo resumido de decisión local de riego. Elaboración propia, véase Anexo~\ref{sec:uso_ia_generativa_tfg}.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.70\textwidth]{figs_png/flujo_supervision_raspberry.png}
        \caption{Flujo de supervisión local en Raspberry Pi. Elaboración propia, véase Anexo~\ref{sec:uso_ia_generativa_tfg}.}
    \label{fig:flujo_supervision_raspberry}
\end{figure}
    
Este flujo representa el papel de la Raspberry Pi como sistema de supervisión: trabaja sobre datos ya publicados, los almacena localmente y los pone a disposición de la API y del \textit{dashboard}.
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.7\textwidth]{figs_png/flujo_fallos_resumido.png}
        \caption{Flujo resumido ante fallos de sensor o comunicación. Elaboración propia, véase Anexo~\ref{sec:uso_ia_generativa_tfg}., véase Anexo~\ref{sec:uso_ia_generativa_tfg}.}
    \label{fig:flujo_fallos_resumido}
\end{figure}

La prioridad del sistema ante condiciones no fiables es mantener el estado seguro: relé apagado, bloqueo de riegos repetidos y actuación limitada temporalmente.
...
\hline
\rowcolor{URJCRed}
\TVHead{Paso} & \TVHead{Elemento} & \TVHead{Acción realizada} \\
\hline
...
\hline
3 & Relé y bomba & Ejecutan el riego cuando se cumplen las condiciones definidas por el \textit{firmware} y durante un tiempo limitado. \\
\hline
...
\hline
\rowcolor{URJCRed}
\TVHead{Bloque} & \TVHead{Responsabilidad principal} & \TVHead{Papel en el sistema} \\
\hline
Sensores & Medir variables del suelo y del entorno. & Adquisición de datos físicos. \\
\hline
Arduino UNO & Leer sensores, validar datos, decidir riego y controlar el relé. & Control local de estación. \\
\hline
Relé y bomba & Ejecutar físicamente el riego cuando el \textit{firmware} lo permite. & Actuación física. \\
\hline
A7670E-LASA & Enviar telemetría mediante LTE. & Comunicación remota. \\
\hline
ThingSpeak & Recibir y conservar lecturas publicadas por las estaciones. & \textit{Broker} de telemetría. \\
\hline
Raspberry Pi & Ingerir, almacenar y servir datos al \textit{dashboard}. & Supervisión local. \\
\hline
API FastAPI & Exponer lecturas, histórico, estadísticas y estado. & Capa de consulta HTTP. \\
\hline
\textit{Dashboard} & Presentar información al usuario. & Interfaz web de supervisión. \\
\hline
\end{tabular}
