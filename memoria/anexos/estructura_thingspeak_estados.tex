...
ThingSpeak cumple una función concreta dentro de TierraViva: actuar como \textit{broker} de telemetría entre las estaciones de campo y la Raspberry Pi, recibiendo los datos publicados por las estaciones y permitiendo su consulta posterior desde el sistema de supervisión. Esta decisión simplifica la comunicación entre la estación de campo y el nodo local de supervisión, ya que la estación solo necesita publicar datos y no mantener una conexión directa con la Raspberry Pi.

El riego se decide en el \textit{firmware} de la estación. Por tanto, ThingSpeak no controla la bomba, no modifica umbrales y no envía comandos a Arduino.
...
\hline
Clave de escritura incorrecta & El canal no recibe nuevas entradas. & Revisar \texttt{THINGSPEAK\_API\_KEY} en el \textit{firmware}. \\
\hline
...
\begin{itemize}
    ...
    \item no se envían órdenes de riego desde el \textit{dashboard};
    \item no existe canal descendente de comandos hacia la estación.
\end{itemize}

Esta limitación es deliberada. La actuación sobre una bomba implica riesgos eléctricos e hidráulicos, por lo que la versión actual prioriza una lógica local con apagado explícito, \textit{cooldown} y comportamiento seguro ante lecturas no válidas.
