...
Las tareas de montaje eléctrico, instalación en Raspberry Pi y configuración de ThingSpeak se documentan en el Anexo \ref{anexo:manual_instalacion_sistema}. La referencia detallada de los \textit{endpoints} de la API se recoge en el Anexo \ref{anexo:manual_api}.
...
\hline
\rowcolor{URJCRed}
\TVHead{Perfil} & \TVHead{Uso principal del sistema} \\
\hline
Usuario general técnico & Consultar el estado de la parcela, comprobar si el suelo está seco o húmedo y revisar si la estación está actualizando datos. \\
\hline
...
El \textit{dashboard} está organizado en bloques visuales para que el estado del sistema pueda interpretarse rápidamente. La Figura~\ref{fig:manual_dashboard_vista_general} muestra una vista general de la interfaz final, y la Tabla~\ref{tab:zonas_dashboard} resume las zonas principales.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{figs/dashboard_final_vista_general.jpg}
    \caption{Vista general del \textit{dashboard} web de TierraViva. Captura propia.}
    \label{fig:manual_dashboard_vista_general}
\end{figure}
...
\hline
\rowcolor{URJCRed}
\TVHead{Zona del dashboard} & \TVHead{Información mostrada} \\
\hline
...
Si el sistema tiene más de una estación configurada, el \textit{dashboard} permite seleccionar la estación activa mediante un desplegable. En la configuración actual están disponibles las estaciones \texttt{A} y \texttt{B}. El selector se encuentra en la zona superior derecha de la interfaz, junto al indicador de conexión.
...
\hline
\rowcolor{URJCRed}
\TVHead{Variable} & \TVHead{Unidad} & \TVHead{Interpretación} \\
\hline
...
Si una variable aparece como ``---'', indica ausencia de dato disponible para esa lectura. Esta situación puede deberse a una lectura pendiente, a un valor no recibido desde el sensor o a la falta de registros válidos almacenados para la estación seleccionada.
...
\hline
\rowcolor{URJCRed}
\TVHead{Estado} & \TVHead{Condición} & \TVHead{Interpretación} \\
\hline
...
\hline
Sin dato & Ausencia de lectura válida & Interpretación hídrica pendiente hasta recibir una lectura correcta. \\
\hline
\end{tabular}
...
El \textit{dashboard} muestra la última lectura junto con su antigüedad. Esta información permite saber si la estación está actualizando datos de forma reciente o si se ha perdido continuidad en la telemetría.
...
\hline
Sin datos & Ausencia de lecturas almacenadas & Publicación pendiente de la estación o recuperación de datos pendiente desde la Raspberry Pi. \\
\hline
\end{tabular}
...
\hline
\rowcolor{URJCRed}
\TVHead{Valor} & \TVHead{Estado mostrado} & \TVHead{Interpretación} \\
\hline
0 & OFF & El relé está apagado y la bomba permanece desalimentada en el momento de la lectura publicada. \\
\hline
1 & Activo & El relé está activo y el sistema de riego puede estar funcionando. \\
\hline
Sin dato & Sin dato & Ausencia de información válida del relé. \\
\hline
\end{tabular}
...
\hline
\rowcolor{URJCRed}
\TVHead{Código} & \TVHead{Evento} & \TVHead{Interpretación} \\
\hline
100 & \texttt{idle} & Estación en reposo, con riego inactivo. \\
\hline
...
\hline
\rowcolor{URJCRed}
\TVHead{Código} & \TVHead{Etiqueta} & \TVHead{Interpretación} \\
\hline
200 & OK & Funcionamiento correcto. \\
\hline
204 & no riego & Ciclo finalizado con riego inactivo. \\
\hline
...
Las gráficas se construyen a partir de las últimas lecturas almacenadas en SQLite. Ante un histórico insuficiente, la gráfica mostrará un mensaje indicando que la información disponible todavía es limitada.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figs/dashboard_graficas_estado.jpg}
    \caption{Gráficas de evolución reciente del \textit{dashboard}, incluyendo variables ambientales, humedad del suelo y códigos de estado publicados por la estación. Captura propia.}
    \label{fig:manual_dashboard_graficas_estado}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figs/dashboard_resumen_historico.jpg}
    \caption{Resumen histórico del \textit{dashboard}, con número de muestras, ventana temporal y valores agregados de las variables medidas. Captura propia.}
    \label{fig:manual_dashboard_resumen_historico}
\end{figure}
...
\hline
\rowcolor{URJCRed}
\TVHead{Dato histórico} & \TVHead{Utilidad} \\
\hline
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figs/dashboard_tabla_ultimas_lecturas.jpg}
    \caption{Tabla de últimas lecturas almacenadas y mostradas por el \textit{dashboard}. Captura propia.}
    \label{fig:manual_dashboard_tabla_ultimas_lecturas}
\end{figure}
...
\hline
\rowcolor{URJCRed}
\TVHead{Situación observada} & \TVHead{Interpretación probable} & \TVHead{Actuación recomendada} \\
\hline
El \textit{dashboard} muestra ``Sin datos'' & Ausencia de lecturas almacenadas para la estación seleccionada. & Comprobar que la estación publica en ThingSpeak y que \texttt{main.py} está ejecutándose. \\
\hline
...
\hline
Las gráficas muestran información vacía & Histórico insuficiente o respuesta de la API sin lecturas disponibles. & Esperar nuevas lecturas y comprobar \texttt{/api/readings}. \\
\hline
Los valores permanecen constantes durante mucho tiempo & Posible ausencia de nuevas publicaciones o procesamiento de entradas duplicadas por parte del ingestor. & Revisar \texttt{entry\_id} en ThingSpeak, \textit{logs} de \texttt{main.py} y estado de la estación. \\
\hline
\end{tabular}
...
\begin{itemize}
    ...
    \item realizar las pruebas iniciales de bomba con supervisión directa;
    ...
    \item registrar cualquier modificación de umbrales;
    \item compartir únicamente capturas libres de claves API;
...
El \textit{dashboard} de TierraViva permite supervisar de forma sencilla el estado de una o varias estaciones de riego inteligente desde una página web servida por la Raspberry Pi. La interfaz resume las variables principales, clasifica el estado de humedad, muestra la frescura de los datos, representa gráficas recientes y permite revisar el histórico almacenado localmente.
