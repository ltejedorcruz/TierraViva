...
El dashboard de TierraViva puede ser utilizado por dos tipos principales de usuario:
...
\hline
Usuario técnico & Verificar lecturas, revisar estado de conexión, comprobar eventos de sistema, analizar histórico y detectar posibles problemas de sensores, comunicación o backend. \\
\hline
\end{tabular}
...
Una vez arrancado el sistema en la Raspberry Pi, el usuario accede al dashboard desde un navegador web conectado a la misma red local.
...
El dashboard está organizado en bloques visuales para que el estado del sistema pueda interpretarse rápidamente. La Tabla \ref{tab:zonas_dashboard} resume las zonas principales de la interfaz.
...
\hline
\end{tabular}
\caption{Zonas principales del dashboard TierraViva.}
\label{tab:zonas_dashboard}
\end{table}

\section{Selección de estación}

Si el sistema tiene más de una estación configurada, el dashboard permite seleccionar la estación activa mediante un desplegable. En la configuración actual se contemplan las estaciones \texttt{A} y \texttt{B}.

Al seleccionar una estación, el dashboard actualiza:
...
\hline
\end{tabular}
\caption{Lecturas actuales mostradas por el dashboard.}
...
El dashboard clasifica la humedad del suelo en tres estados: seco, normal y húmedo. Esta clasificación se calcula utilizando los umbrales configurados en la API.
...
En la configuración por defecto, los umbrales utilizados por el dashboard son 35\% para el límite bajo y 45\% para el límite alto. Estos valores pueden modificarse en el fichero de configuración \texttt{.env} mediante las variables \texttt{SOIL\_LOW\_THRESHOLD} y \texttt{SOIL\_HIGH\_THRESHOLD}.

\section{Estado de conexión y frescura de datos}

El dashboard no solo muestra la última lectura, sino también su antigüedad. Esto permite saber si la estación está actualizando datos de forma reciente o si se ha perdido continuidad en la telemetría.
...
El dashboard muestra el estado del relé recibido desde la estación. Este valor procede de la telemetría publicada en ThingSpeak y permite saber si el sistema de actuación está apagado o activo en el momento de la lectura.
...
El usuario debe tener en cuenta que el dashboard muestra el estado recibido en la última lectura almacenada. Si existe retraso de comunicación, el estado físico real puede haber cambiado antes de que el dashboard se actualice.

\section{Eventos de sistema}

El campo de evento permite interpretar el estado interno de la estación. La Tabla \ref{tab:eventos_sistema_usuario} recoge los códigos utilizados por el dashboard.
...
\hline
\end{tabular}
\caption{Eventos de sistema mostrados en el dashboard.}
...
\hline
429 & cooldown & La estación evita un nuevo riego por seguridad temporal. \\
\hline
\end{tabular}
\caption{Códigos de diagnóstico mostrados en el dashboard.}
\label{tab:codigos_diagnostico_usuario}
\end{table}

\section{Gráficas}

El dashboard incluye gráficas de evolución reciente para las variables almacenadas. Estas gráficas permiten detectar tendencias de forma rápida, por ejemplo:
...
\begin{itemize}
    ...
    \item repetición de eventos de error o cooldown.
\end{itemize}
...
\begin{enumerate}
    ...
    \item abrir el dashboard desde un navegador;
...
\hline
El dashboard muestra ``Sin datos'' & No hay lecturas almacenadas para la estación seleccionada. & Comprobar que la estación publica en ThingSpeak y que \texttt{main.py} está ejecutándose. \\
\hline
El estado aparece como ``Sin actualización'' & La última lectura es demasiado antigua. & Revisar alimentación de la estación, cobertura LTE, canal ThingSpeak y logs de Raspberry Pi. \\
\hline
La humedad aparece como ``Seco'' & El valor está por debajo del umbral bajo. & Verificar si la estación ejecuta riego local o si se mantiene bloqueada por cooldown. \\
\hline
Aparece código 429 & La estación está en periodo de seguridad entre riegos. & Esperar a que finalice el cooldown y revisar si el valor de humedad sigue siendo bajo. \\
\hline
...
\hline
Los valores no cambian durante mucho tiempo & Puede no haber nuevas publicaciones o el ingestor está ignorando duplicados. & Revisar entry\_id en ThingSpeak, logs de \texttt{main.py} y estado de la estación. \\
\hline
\end{tabular}
...
\begin{itemize}
    ...
    \item revisar periódicamente el estado de conexión del dashboard;
    ...
    \item revisar logs si aparecen estados de error o datos obsoletos;
    \item contrastar el dashboard con una observación física durante pruebas de riego.
\end{itemize}

\section{Conclusión}

El dashboard de TierraViva permite supervisar de forma sencilla el estado de una o varias estaciones de riego inteligente. La interfaz resume las variables principales, clasifica el estado de humedad, muestra la frescura de los datos, representa gráficas recientes y permite revisar histórico almacenado en la Raspberry Pi.

El usuario debe interpretar el sistema como una herramienta de monitorización y apoyo a la validación del prototipo. La actuación sobre el riego se realiza en la estación Arduino mediante lógica local, mientras que el dashboard permite comprobar si las lecturas, eventos y estados publicados son coherentes con el funcionamiento esperado.