...
El objetivo es dejar constancia de los riesgos más relevantes para el desarrollo, las pruebas y una posible evolución futura del prototipo, centrando el análisis en aquellos aspectos con mayor impacto sobre el funcionamiento, la seguridad y la validación del sistema.
...
\hline
\rowcolor{URJCRed}
\TVHead{Probabilidad / Impacto} & \TVHead{Bajo} & \TVHead{Medio} & \TVHead{Alto} \\
\hline
...
\hline
\rowcolor{URJCRed}
\TVHead{Código} & \TVHead{Riesgo} & \TVHead{Prob.} & \TVHead{Imp.} & \TVHead{Nivel} \\
\hline
...
\hline
R04 & Indisponibilidad temporal de ThingSpeak o límites de uso de la plataforma. & M & M & \cellcolor{yellow!30}Medio \\
\hline
R05 & Activación incorrecta o excesiva de la bomba por error de lógica o lectura anómala. & M & A & \cellcolor{red!25}Alto \\
\hline
...
\hline
R09 & Inconsistencias entre \textit{firmware}, \textit{backend}, API y \textit{dashboard} durante la evolución del código. & M & M & \cellcolor{yellow!30}Medio \\
\hline
...
\end{table}
\begin{flushleft}
\footnotesize\textit{Nota:} B = baja/bajo, M = media/medio y A = alta/alto, según se trate de probabilidad, impacto o nivel de riesgo.
\end{flushleft}
...
\hline
\rowcolor{URJCRed}
\TVHead{Bloque} & \TVHead{Riesgos asociados} & \TVHead{Nivel} \\
\hline
...
La humedad del suelo es la variable principal para decidir el riego. Una lectura incorrecta puede provocar una actuación innecesaria o impedir el riego cuando el suelo está seco.
...
\begin{itemize}
    \item limitación del tiempo máximo de riego por \textit{firmware};
...
Las pruebas en exterior pueden exponer la electrónica a humedad, polvo, salpicaduras o tirones de cable. Al tratarse de un prototipo académico funcional, estas condiciones deben controlarse durante los ensayos y considerarse en la evolución hacia versiones más robustas.
...
La gestión de riesgos se conecta directamente con el plan de pruebas. Cada riesgo relevante se asocia a una comprobación que permite verificar la eficacia de las medidas adoptadas durante la validación.
...
\hline
\rowcolor{URJCRed}
\TVHead{Riesgo} & \TVHead{Prueba asociada} & \TVHead{Evidencia esperada} \\
\hline
...
\hline
R03 & Prueba de comunicación LTE y recuperación tras pérdida puntual de enlace. & Registro de publicación en ThingSpeak o \textit{logs} de fallo y recuperación de conectividad. \\
\hline
R04 & Prueba de consulta a ThingSpeak y comportamiento ante ausencia temporal de nuevas entradas. & Última lectura disponible, estado de dato antiguo u observación registrada en la API o el \textit{dashboard}. \\
\hline
...
\hline
R09 & Prueba de integración \textit{firmware}--ThingSpeak--Raspberry--\textit{dashboard}. & Lecturas coherentes desde la estación hasta la interfaz web. \\
\hline
R10 & Planificación de prueba en entorno real o controlado. & Evidencia de ejecución de la prueba, fotografías, capturas o registro de limitaciones logísticas. \\
\hline
\end{tabular}
...
Tras aplicar las medidas anteriores, permanecen algunos riesgos residuales propios de un prototipo físico conectado a servicios externos y probado en un entorno parcialmente controlado.
...
Estos riesgos delimitan correctamente el alcance de TierraViva como prototipo funcional de Trabajo de Fin de Grado y orientan posibles mejoras futuras, especialmente en robustez física, autonomía, protección ambiental, seguridad de credenciales y tolerancia a fallos.
...
Las medidas adoptadas reducen estos riesgos y hacen que el prototipo sea más seguro, trazable y defendible. Además, este análisis conecta directamente los riesgos con el plan de pruebas y permite validar el sistema también ante condiciones representativas de fallo, degradación de comunicación, lecturas anómalas o exposición controlada en entorno exterior.