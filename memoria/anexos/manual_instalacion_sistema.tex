...
\begin{itemize}
    ...
    \item dashboard web accesible desde navegador.
\end{itemize}
...
\begin{itemize}
    ...
    \item Arduino IDE para cargar el firmware de estación;
...
Las claves reales no deben incluirse en la memoria ni subirse al repositorio. En la documentación se utilizarán siempre marcadores como \texttt{<READ\_KEY\_A>} o \texttt{<WRITE\_KEY\_A>}.
...
\section{Carga del firmware de estación}

El firmware de la estación se encuentra en:
...
\hline
\end{tabular}
\caption{Parámetros principales que deben revisarse antes de cargar el firmware.}
\label{tab:parametros_firmware_instalacion}
\end{table}

\subsection{Librerías empleadas por el firmware}
\label{subsec:librerias_firmware_estacion}

El firmware de la estación remota utiliza varias librerías de Arduino y de C/C++ para gestionar la comunicación con sensores, el módem LTE y el tratamiento de cadenas. Estas dependencias no añaden funcionalidad aislada, sino que permiten implementar de forma ordenada los bloques principales del sketch: lectura I2C, comunicación serie con el A7670E-LASA, adquisición de medidas ambientales y construcción de peticiones HTTP.
...
\hline
\end{tabular}
\caption{Librerías principales utilizadas por el firmware de estación.}
...
La carga del firmware se realiza desde Arduino IDE:
...
\begin{figure}[H]
    ...
    \caption{Selección de la placa \textit{Arduino UNO} en Arduino IDE antes de cargar el firmware de la estación.}
...
\begin{figure}[H]
    ...
    \caption{Selección del puerto serie correspondiente a la placa Arduino conectada por USB en Arduino IDE.}
...
\begin{verbatim}
TIERRAVIVA_HOME=/home/pi/TierraViva
...
El fichero \texttt{.env} no debe subirse al repositorio. Si accidentalmente se publica una clave real, debe regenerarse en ThingSpeak.

\section{Preparación del dashboard}

El dashboard fuente se encuentra en el directorio:

\begin{verbatim}
dashboard/
\end{verbatim}

La API sirve la interfaz desde:

\begin{verbatim}
frontend/index.html
\end{verbatim}

Por tanto, antes de arrancar el sistema debe copiarse el dashboard al directorio \texttt{frontend/}:

\begin{verbatim}
mkdir -p frontend
rsync -a dashboard/ frontend/
\end{verbatim}
...
\begin{verbatim}
frontend/index.html
frontend/css/tierraviva.css
frontend/js/tierraviva.js
\end{verbatim}

\section{Instalación mediante script}

Antes de utilizar el script debe concederse permiso de ejecución:
...
Tras la instalación, se recomienda repetir la copia del dashboard para asegurar que el frontend servido por la API está actualizado:
...
\begin{itemize}
    ...
    \item \texttt{api.py}, encargado de servir la API y el dashboard web.
\end{itemize}
...
La descripción completa de endpoints, parámetros y respuestas se recoge en el Anexo \ref{anexo:manual_api}.

\section{Acceso al dashboard}

Una vez arrancada la API y copiado el dashboard en \texttt{frontend/}, la interfaz web se abre desde:
...
El dashboard debe mostrar la estación seleccionada, la última lectura disponible, el estado de frescura, variables ambientales, estado de relé, eventos, gráficas y tabla de lecturas recientes.
...
\hline
Estación encendida & Arduino, sensores y módem arrancan sin reinicios.\\
\hline
...
\hline
Dashboard visible & La interfaz muestra datos de la estación.\\
\hline
Relé sin bomba & El relé conmuta y vuelve a apagado.\\
\hline
...
\hline
No aparecen datos en ThingSpeak & Clave de escritura incorrecta, APN incorrecto o fallo LTE. & Revisar firmware, SIM, antena, APN y monitor serie. \\
\hline
La Raspberry no recupera datos & Canal o clave de lectura incorrectos. & Revisar \texttt{THINGSPEAK\_CHANNEL\_A/B} y \texttt{THINGSPEAK\_READ\_KEY\_A/B}. \\
\hline
La API no arranca & Entorno virtual o dependencias no preparados. & Ejecutar \texttt{./tierraviva.sh install}. \\
\hline
El dashboard no carga & No existe \texttt{frontend/index.html}. & Ejecutar \texttt{rsync -a dashboard/ frontend/}. \\
\hline
No hay registros en SQLite & La estación no ha publicado o el ingestor no está activo. & Revisar ThingSpeak y logs de \texttt{main.py}. \\
\hline
El relé se activa al arrancar & Lógica invertida o inicialización incorrecta. & Comprobar configuración del firmware y probar sin bomba. \\
\hline
La bomba no se apaga & Error de cableado o uso del contacto incorrecto del relé. & Usar contacto normalmente abierto y validar apagado por firmware. \\
\hline
...
La puesta en marcha de TierraViva debe realizarse por bloques: primero estación y sensores, después publicación en ThingSpeak, más tarde ingesta en Raspberry Pi, base de datos, API, dashboard y finalmente actuación sobre relé o bomba. Esta secuencia permite aislar errores y evita que un fallo de montaje, configuración o comunicación quede oculto dentro de una prueba completa.

El procedimiento descrito en este anexo no sustituye a los anexos técnicos específicos, sino que los ordena en una secuencia práctica de instalación y validación inicial.