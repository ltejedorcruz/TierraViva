...
\begin{figure}[H]
    ...
    \caption{Selección de la placa \textit{Arduino UNO} en Arduino IDE antes de cargar el \textit{firmware} de la estación.}
...
Por tanto, antes de arrancar el sistema debe copiarse el contenido de \texttt{dashboard/} al directorio \texttt{frontend/}:
...
No se requiere la existencia de carpetas \texttt{frontend/css/} ni \texttt{frontend/js/}, ya que los estilos y la lógica JavaScript están integrados en el propio fichero \texttt{index.html}. Esta organización simplifica el despliegue del prototipo y reduce el riesgo de rutas estáticas inconsistentes durante las pruebas.
...
Tras la instalación, se recomienda comprobar que el contenido de \texttt{dashboard/} está sincronizado con \texttt{frontend/}, especialmente el fichero \texttt{index.html} y el directorio \texttt{images/}:
...
\hline
No aparecen datos en ThingSpeak & Clave de escritura incorrecta, APN incorrecto o fallo LTE. & Revisar \textit{firmware}, SIM, antena, APN y monitor serie. \\
\hline
...
\hline
El \textit{dashboard} no carga & No existe \texttt{frontend/index.html} o no se ha sincronizado el contenido de \texttt{dashboard/}. & Ejecutar \texttt{rsync -a dashboard/ frontend/} y comprobar que existen \texttt{frontend/index.html} y \texttt{frontend/images/}. \\
\hline
El \textit{dashboard} carga sin logotipos o imágenes & No se ha copiado correctamente el directorio \texttt{images/}. & Repetir \texttt{rsync -a dashboard/ frontend/} y revisar que existen los ficheros gráficos en \texttt{frontend/images/}. \\
\hline
No hay registros en SQLite & La estación no ha publicado o el ingestor no está activo. & Revisar ThingSpeak y \textit{logs} de \texttt{main.py}. \\
\hline
...
\hline
La bomba no se apaga & Error de cableado o uso del contacto incorrecto del relé. & Usar contacto normalmente abierto y validar apagado por \textit{firmware}. \\
\hline
...
La puesta en marcha de TierraViva debe realizarse por bloques: primero estación y sensores, después publicación en ThingSpeak, más tarde ingesta en Raspberry Pi, base de datos, API, \textit{dashboard} y finalmente actuación sobre relé o bomba. Esta secuencia permite aislar errores y evita que un fallo de montaje, configuración o comunicación quede oculto dentro de una prueba completa.
