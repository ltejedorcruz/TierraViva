...
Este capítulo concreta el diseño de TierraViva a partir del marco técnico descrito en el capítulo anterior. Aquí se fijan los requisitos del sistema, se explica la metodología seguida y se documentan las decisiones que han guiado el diseño del prototipo, tales como las variables a medir en el entorno físico, la tecnología de comunicación inalámbrica empleada, el rol de cada elemento dentro de la solución completa y las condiciones que deben cumplirse para actuar sobre el riego.

A diferencia del estado del arte, el análisis se centra ya en el proyecto desarrollado. El capítulo se organiza de forma progresiva: primero se presenta la metodología de trabajo y se definen los requisitos del sistema; después se justifican las principales decisiones tecnológicas; y, finalmente, se sintetiza la arquitectura general resultante. Algunas decisiones se tomaron de forma incremental, a medida que se validaban sensores, comunicaciones y software de supervisión, por lo que la memoria recoge también los reajustes realizados durante el desarrollo.

\section{Metodología de trabajo}
\label{sec:metodologia_trabajo}

La metodología seguida en TierraViva se ha basado en un desarrollo incremental por bloques. Este enfoque resulta adecuado para un sistema IoT que combina sensórica, comunicaciones, almacenamiento, visualización y actuación física, ya que permite validar cada parte por separado antes de integrarla en el sistema completo.

El trabajo no se desarrolló como una implementación lineal de una arquitectura cerrada desde el inicio. Durante el proyecto se realizaron pruebas parciales, se descartaron alternativas, se reajustó la arquitectura y se documentaron incidencias técnicas. Esta forma de trabajo permitió reducir el riesgo de integración y justificar las decisiones adoptadas.
...
\hline
\rowcolor{URJCRed}
\TVHead{Fase} & \TVHead{Objetivo principal} \\
\hline
Análisis y revisión tecnológica & Identificar el problema, revisar alternativas de sensorización, comunicación, plataformas embebidas y supervisión IoT. \\
\hline
Diseño conceptual & Definir requisitos, arquitectura por bloques, separación entre control local y supervisión, y criterios de seguridad funcional. \\
\hline
Prototipado parcial & Validar sensores, plataforma de control, comunicación, almacenamiento, servicios de consulta e interfaz de supervisión mediante pruebas separadas. \\
\hline
Reajuste de arquitectura & Sustituir el enfoque local inicial por estaciones remotas con comunicación inalámbrica, plataforma intermedia de telemetría y decisión local en \textit{firmware}. \\
\hline
Integración y validación & Conectar sensórica, comunicación, almacenamiento, servicios de consulta, interfaz de supervisión, relé, bomba y pruebas funcionales. \\
\hline
Documentación y cierre & Registrar decisiones, problemas, evidencias, anexos técnicos, resultados, limitaciones y conclusiones. \\
\hline
\end{tabular}
\caption{Resumen de la metodología incremental seguida en TierraViva.}
\label{cuadro:metodologia_fases}
\end{table}

El cronograma detallado del proyecto se recoge en el Anexo~\ref{anexo:cronograma}, y el seguimiento de decisiones técnicas se documenta en el Anexo~\ref{anexo:seguimiento_decisiones}. En este capítulo interesa únicamente fijar el criterio metodológico: avanzar por bloques, validar antes de integrar y reajustar la arquitectura cuando las pruebas lo hicieron necesario.
...
El sistema se plantea a partir de estaciones de campo capaces de obtener medidas del entorno, evaluar localmente reglas sencillas de riego y enviar la información a una plataforma intermedia. En el prototipo se contemplan dos estaciones sensoras-actuadoras, denominadas TierraViva\_A y TierraViva\_B, con el fin de validar que la arquitectura puede diferenciar el origen de las lecturas y supervisar más de un nodo de campo. A partir de esos datos, el sistema de supervisión debe poder registrar el histórico, visualizar el estado del prototipo y mostrar las actuaciones realizadas por cada estación. La estación de campo debe ejecutar la actuación de forma autónoma y segura, evitando activaciones indefinidas, riegos repetidos o estados peligrosos ante lecturas inválidas, errores de comunicación o fallos eléctricos.

Para facilitar la trazabilidad del sistema, los requisitos funcionales se identifican con el prefijo RF (requisito funcional). 

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figs_png/figura_3_1_requisitos_funcionales.png}
    \caption{Relación entre los requisitos funcionales y el ciclo completo de funcionamiento del sistema. TierraViva debe medir, publicar telemetría, registrar histórico, visualizar el estado, decidir localmente y actuar sobre el riego de forma segura. Elaboración propia, véase Anexo~\ref{sec:uso_ia_generativa_tfg}.}
    \label{fig:cap3_requisitos_funcionales}
\end{figure}

En este capítulo se utiliza el término interfaz de supervisión para referirse, de forma general, a la capa visual que permitirá consultar lecturas recientes, estado del sistema, históricos y eventos relevantes del prototipo.
...
\hline
RF-04 & Envío remoto de telemetría & Las estaciones de campo deben enviar sus medidas a una plataforma remota para que puedan ser consultadas posteriormente. & Prueba de comunicación remota y recepción en plataforma \textit{cloud}. \\
\hline
RF-05 & Identificación de estación & Cada lectura debe poder asociarse a la estación de origen, permitiendo distinguir los datos de las estaciones TierraViva\_A y TierraViva\_B o de futuros nodos añadidos. & Comprobación de registros diferenciados por estación. \\
\hline
RF-06 & Almacenamiento histórico & El sistema debe registrar las lecturas recibidas para conservar un histórico de funcionamiento. & Verificación de inserciones en la base de datos local. \\
\hline
RF-07 & Visualización del estado & El sistema debe ofrecer una interfaz de consulta que permita ver las últimas lecturas y el estado general del prototipo. & Prueba de interfaz web de supervisión en navegador. \\
\hline
RF-08 & Decisión local de riego & Cada estación debe evaluar localmente si las condiciones medidas justifican una actuación sobre el riego. & Prueba de regla local de humedad, \textit{cooldown} y estado seguro. \\
\hline
RF-09 & Actuación local sobre el riego & La estación debe accionar físicamente el sistema de riego mediante relé y bomba de agua durante un tiempo limitado por \textit{firmware}. & Prueba controlada con relé y bomba/carga de prueba. \\
\hline
RF-10 & Publicación de estado de actuación & La estación debe publicar información suficiente para identificar el estado del actuador, el evento principal del sistema y posibles condiciones de diagnóstico, como actuación, reposo, error, estado seguro o bloqueo temporal. & Comprobación de la publicación e interpretación de estados en la plataforma intermedia de telemetría. \\
\hline
RF-11 & Seguridad de funcionamiento & El sistema debe desactivar o mantener apagada la bomba ante error de sensor, lectura inválida, fallo de comunicación, reinicio, \textit{cooldown} activo o tiempo máximo excedido. & Pruebas de modo seguro, reinicio y límites de actuación. \\
\hline
...
El sistema debe mantener un equilibrio entre ambición técnica y viabilidad. Por ello, se plantea como una solución suficientemente robusta, ordenada y reproducible como para demostrar el funcionamiento completo de la arquitectura propuesta dentro del alcance académico del TFG. Para facilitar su trazabilidad, los requisitos no funcionales se identifican con el prefijo RNF (requisito no funcional). La Tabla~\ref{cuadro:requisitos_no_funcionales} resume los principales requisitos no funcionales considerados.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figs/figura_3_2_requisitos_no_funcionales.png}
    \caption{Requisitos no funcionales considerados en el diseño del prototipo. Estos criterios permiten evaluar la calidad técnica del sistema más allá de las funciones implementadas. Elaboración propia, véase Anexo~\ref{sec:uso_ia_generativa_tfg}.}
...
Las especificaciones temporales (ET) no se plantean como valores rígidos e inmodificables, sino como parámetros iniciales de diseño. Durante el desarrollo y la validación del prototipo pueden ajustarse en función de los resultados obtenidos, siempre manteniendo dos criterios principales: evitar comunicaciones innecesarias y garantizar que cualquier actuación sobre el riego quede limitada en el tiempo.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{figs/figura_3_3_especificaciones_temporales.png}
    \caption{Especificaciones temporales y operativas iniciales del sistema. Elaboración propia, véase Anexo~\ref{sec:uso_ia_generativa_tfg}.}
...
\hline
ET-03 & Frecuencia de actualización de la interfaz de supervisión & 3--5 s & La interfaz consulta el estado con mayor frecuencia que la llegada real de nuevos datos. \\
\hline
...
\hline
ET-05 & Tiempo máximo de bomba activa & Limitado en pruebas; ajustable en \textit{firmware} & La activación de la bomba queda limitada por \textit{firmware} para evitar riego indefinido, sobrecarga del actuador o consumo innecesario de agua ante errores de control. \\
\hline
...
\hline
ET-07 & Tiempo máximo sin dato reciente en supervisión & Configurable & El sistema de supervisión y la interfaz web deben identificar lecturas antiguas u \textit{offline} cuando no se reciben publicaciones recientes de una estación. \\
\hline
...
Los parámetros temporales se definen como valores configurables del \textit{firmware}. Durante las pruebas pueden emplearse intervalos reducidos para acelerar la validación, mientras que en un funcionamiento más estable conviene utilizar periodos más conservadores. En particular, el tiempo máximo de bomba activa no debe interpretarse como una recomendación agronómica general, sino como un límite de seguridad del prototipo. Su valor debe ser deliberadamente conservador durante la validación y ajustarse posteriormente a partir de pruebas de caudal, tipo de cultivo, volumen de agua aplicado y evolución real de la humedad del suelo. Lo importante desde el punto de vista de diseño no es fijar un único valor definitivo, sino garantizar ciclos controlados, detección de datos antiguos en supervisión y actuación de la bomba siempre limitada temporalmente.

La actualización de la interfaz de supervisión puede realizarse con mayor frecuencia que la publicación de las estaciones, pero no debe interpretarse como una nueva medida en cada refresco. La actuación física queda gobernada por el \textit{firmware} de la estación, que debe mantener la bomba apagada ante lecturas inválidas, reinicios, pérdida de comunicación o estados no verificables.
...
\begin{figure}[H]
    ...
    \caption{Variables seleccionadas para la monitorización de TierraViva. La humedad del suelo se utiliza como variable principal para la decisión local de riego, mientras que temperatura, humedad relativa, presión y luminosidad aportan contexto ambiental. Elaboración propia, véase Anexo~\ref{sec:uso_ia_generativa_tfg}.}
...
\hline
Capacidad software & \textit{firmware} sencillo y tareas embebidas. & Sistema operativo, API, base de datos e interfaz web de supervisión. \\
\hline
...
LTE con el módulo A7670E-LASA se eligió como alternativa más adecuada para validar el sistema completo. Esta opción permite que cada estación publique datos directamente en Internet mediante red móvil y peticiones HTTP, sin desplegar \textit{gateway} propio. A cambio, introduce dependencia de cobertura móvil, tarjeta SIM, consumo energético superior y una alimentación más exigente para el módem.
...
\hline
Infraestructura & Puede requerir \textit{gateway}, servidor LoRaWAN o receptor propio. & No requiere \textit{gateway} propio; usa red móvil existente. \\
\hline
Integración \textit{cloud} & Necesita pasarela o \textit{backend} intermedio. & Integración directa mediante HTTP. \\
\hline
...
\section{Elección de plataforma \textit{cloud}: ThingSpeak}
\label{sec:eleccion_plataforma_cloud}

La plataforma \textit{cloud} debía actuar como elemento intermedio entre las estaciones remotas y la Raspberry Pi. En términos funcionales, se necesitaba un \textit{broker} de telemetría capaz de recibir los datos publicados por las estaciones y ponerlos a disposición del sistema de supervisión sin exigir una conexión física directa con los nodos de campo. En TierraViva, este papel se asigna a ThingSpeak.

Se valoraron alternativas como \textit{brokers} MQTT accesibles desde Internet, Ubidots, Adafruit IO, Blynk, ThingsBoard o una plataforma propia. Sin embargo, para el alcance del TFG se buscaba una solución sencilla, verificable y compatible con dispositivos embebidos. ThingSpeak se seleccionó como plataforma IoT y \textit{broker} de telemetría porque ofrece una estructura basada en canales y campos, permite escribir datos mediante una API documentada sobre HTTP y facilita una primera visualización de las medidas publicadas \cite{thingspeak_docs,thingspeak_write_data}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.78\textwidth]{figs/figura_3_9_thingspeak_capa_intermedia.png}
    \caption{Uso de ThingSpeak como \textit{broker} de telemetría entre estaciones remotas y sistema de supervisión. La plataforma organiza la telemetría y los estados por canales y campos, permitiendo su consulta posterior desde Raspberry Pi. Elaboración propia, véase Anexo~\ref{sec:uso_ia_generativa_tfg}.}
    \label{fig:cap3_thingspeak_capa_intermedia}
\end{figure}

En TierraViva se utilizan canales diferenciados para cada estación, lo que permite separar los datos de las dos estaciones sensoras-actuadoras definidas para el prototipo: TierraViva\_A y TierraViva\_B. Cada publicación contiene medidas ambientales, estado del relé, evento de sistema y código auxiliar de diagnóstico. Esta organización facilita la trazabilidad, evita mezclar lecturas de distintas estaciones y permite que la Raspberry Pi consulte los datos de forma independiente.
...
La Raspberry Pi se ha seleccionado como sistema de supervisión software de TierraViva. Su función es consultar la información publicada por las estaciones, registrar un histórico local y ofrecer una interfaz de consulta. Este papel queda claramente diferenciado del de las estaciones de campo, que se encargan de leer directamente los sensores, transmitir las lecturas y decidir localmente la activación de la bomba. En TierraViva, esta separación evita que un fallo del sistema de supervisión afecte directamente a la actuación física sobre el riego.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figs_png/figura_3_10_arquitectura_software_raspberry.png}
    \caption{Arquitectura software de supervisión en Raspberry Pi. La información publicada en la nube se consulta, valida, almacena en SQLite, expone mediante FastAPI y se presenta en el \textit{dashboard} web. Elaboración propia, véase Anexo~\ref{sec:uso_ia_generativa_tfg}.}
    \label{fig:cap3_arquitectura_software_raspberry}
\end{figure}

La arquitectura software de supervisión se apoya en tres bloques principales: almacenamiento local, API y \textit{dashboard}. SQLite se selecciona como base de datos local por su sencillez de despliegue y por ser suficiente para conservar el histórico de lecturas, estados e incidencias del prototipo sin instalar un servidor de base de datos adicional \cite{sqlite_about}. Esta decisión permite que la Raspberry Pi mantenga una memoria propia del sistema, complementaria a los datos publicados en ThingSpeak.

FastAPI se selecciona como \textit{framework} para construir la API de supervisión en Python \cite{fastapi_docs}. Su papel dentro de TierraViva es actuar como capa intermedia entre la base de datos y la interfaz web, evitando que el \textit{dashboard} acceda directamente a SQLite o a los detalles internos de almacenamiento. La API permite consultar datos mediante peticiones HTTP y deja preparada la arquitectura para que otros clientes puedan reutilizar la misma información en futuras ampliaciones.

El \textit{dashboard} web constituye la página web de supervisión del sistema. En TierraViva, este término se utiliza para referirse a una interfaz web alojada en la Raspberry Pi y servida por la aplicación FastAPI, accesible desde un navegador. Esta página integra lecturas recientes, estado del sistema, históricos y eventos relevantes del prototipo, y obtiene la información mediante peticiones HTTP a la API local. Su objetivo es mostrar de forma comprensible las últimas lecturas, el estado de las estaciones y la información operativa publicada por cada nodo.

La implementación concreta de estos bloques, incluyendo la estructura de los \textit{scripts}, los \textit{endpoints} expuestos, el modelo de datos y la evolución desde el prototipo local previo, se desarrolla en el Capítulo~\ref{cap:implementacion_software_raspberry_nube}.
...
\hline
Ingesta desde ThingSpeak & Consulta periódica de telemetría y estado publicados por las estaciones. & Permite alimentar el histórico local y el \textit{dashboard} sin conexión directa con los nodos de campo. \\
\hline
...
\hline
FastAPI & Exposición de datos mediante \textit{endpoints} HTTP. & Facilita crear una API sencilla entre la base de datos y el \textit{dashboard}. \\
\hline
\textit{Dashboard} web & Página web de supervisión servida desde la Raspberry Pi para visualizar medidas, histórico, estado del relé y estado de las estaciones. & Permite supervisar el prototipo desde un navegador sin acceder a la consola ni realizar consultas SQL (\textit{Structured Query Language}). \\
\hline
\end{tabular}
...
La Raspberry conserva un papel relevante como sistema de supervisión: consulta los datos publicados, mantiene un histórico local, expone la API y sirve el \textit{dashboard}. La implementación concreta de estos bloques se desarrolla en el Capítulo~\ref{cap:implementacion_software_raspberry_nube}. La referencia de \textit{endpoints} se recoge en el Anexo~\ref{anexo:manual_api}, y el uso de la interfaz por parte del usuario se documenta en el Anexo~\ref{anexo:manual_usuario}.

\section{Arquitectura general planteada}
\label{sec:arquitectura_general_planteada}

A partir de los requisitos, las especificaciones operativas y las elecciones tecnológicas descritas en los apartados anteriores, la arquitectura general de TierraViva se plantea como un sistema IoT distribuido, formado por estaciones autónomas de campo, ThingSpeak como plataforma intermedia de telemetría y una Raspberry Pi como sistema local de supervisión. El objetivo de esta arquitectura es separar claramente la adquisición de datos, la decisión local, la actuación física, la publicación de telemetría, el almacenamiento histórico y la visualización.
...
\begin{itemize}
    ...
    \item \textbf{Sistema de supervisión}: implementado sobre Raspberry Pi, encargado de consultar datos, almacenarlos, exponer una API y servir el \textit{dashboard}.
...
\begin{figure}[H]
  ...
  \caption{Arquitectura general planteada para TierraViva. Elaboración propia, véase Anexo~\ref{sec:uso_ia_generativa_tfg}.}
\label{fig:arquitectura_general_tierraviva}
\end{figure}

Como puede observarse en la Figura~\ref{fig:arquitectura_general_tierraviva}, la arquitectura planteada no incorpora órdenes de retorno desde el sistema de supervisión hacia las estaciones sensoras-actuadoras. Las estaciones toman sus decisiones localmente a partir de la configuración previamente cargada en el \textit{firmware}, mientras que ThingSpeak y la Raspberry Pi se emplean para telemetría, almacenamiento, consulta y visualización. Esta decisión limita la capacidad de modificar el comportamiento de las estaciones de forma remota, pero reduce la complejidad del primer prototipo y evita que la actuación física dependa de comandos enviados desde capas externas. En futuras versiones podría incorporarse un mecanismo de configuración centralizada, de forma que determinados parámetros u órdenes pudieran definirse desde el sistema de supervisión y transmitirse posteriormente a las estaciones de campo.

La división de responsabilidades es el punto central de esta arquitectura. Las estaciones de campo interactúan con el entorno físico, toman la decisión local de riego y publican telemetría. ThingSpeak actúa como plataforma intermedia de comunicación. La Raspberry Pi consulta los datos, mantiene el histórico, expone la API y sirve el \textit{dashboard}, sin convertirse en el elemento responsable de decidir la activación de la bomba.
...
Funcionalmente, el sistema se organiza en bloques de sensórica, control local, comunicación, supervisión, actuación y alimentación. La sensórica se basa en un sensor capacitivo de humedad de suelo, BME280 y BH1750/GY-302; el control local se realiza con Arduino UNO; la comunicación remota con A7670E-LASA y tarjeta SIM; la supervisión con Raspberry Pi 5; y la actuación mediante relé y bomba de agua de 12 V.
...
\hline
Fallo de alimentación o picos de consumo & Reinicios, pérdida de comunicación o comportamiento inestable del relé. & Regulación con LM2596, comprobación de tensiones, masa común, condensadores de estabilización y diodo asociado a la rama de bomba. \\
\hline
...
\hline
Regla local mal configurada & Activación del riego en condiciones no deseadas. & Umbrales configurables, \textit{cooldown}, pruebas controladas y actuación limitada por \textit{firmware}. \\
\hline
...
\hline
Fallo del software en Raspberry Pi & Pérdida temporal de API, \textit{dashboard} o histórico local. & Separación por módulos, \textit{logs}, \textit{script} de gestión y autonomía de las estaciones para la decisión de riego. \\
\hline
\end{tabular}
