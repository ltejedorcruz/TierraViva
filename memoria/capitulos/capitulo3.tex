...
El término \textit{dashboard} se refiere a un panel web de supervisión que integra, en una misma interfaz, lecturas recientes, estado del sistema, históricos y eventos relevantes del prototipo.
...
\hline
RF-07 & Visualización del estado & El sistema debe ofrecer una interfaz de consulta que permita ver las últimas lecturas y el estado general del prototipo. & Prueba de \textit{dashboard} web en navegador. \\
\hline
...
\hline
RF-10 & Publicación de estado de actuación & La estación debe publicar el estado del relé, el evento de sistema y un código auxiliar de diagnóstico que permitan conocer si se ha producido una actuación, una situación de reposo, un error, un estado seguro o un bloqueo por \textit{cooldown}. & Verificación de \texttt{relay\_state}, \texttt{system\_event} y \texttt{debug\_code} en ThingSpeak. \\
\hline
...
\hline
ET-03 & Frecuencia de actualización del \textit{dashboard} & 3--5 s & La interfaz consulta el estado con mayor frecuencia que la llegada real de nuevos datos. \\
\hline
...
\hline
ET-07 & Tiempo máximo sin dato reciente en supervisión & Configurable & La Raspberry Pi y el \textit{dashboard} deben identificar lecturas antiguas u \textit{offline} cuando no se reciben publicaciones recientes de una estación. \\
\hline
...
\hline
Prototipado parcial & Validar sensores, Arduino, comunicación, almacenamiento, API y \textit{dashboard} mediante pruebas separadas. \\
\hline
Reajuste de arquitectura & Sustituir el enfoque local inicial por estaciones remotas con LTE, ThingSpeak y decisión local en \textit{firmware}. \\
\hline
Integración y validación & Conectar sensórica, comunicación, base de datos, API, \textit{dashboard}, relé, bomba y pruebas funcionales. \\
\hline
...
\hline
Capacidad software & \textit{firmware} sencillo y tareas embebidas. & Sistema operativo, API, base de datos y \textit{dashboard}. \\
\hline
...
\begin{figure}[H]
    ...
    \caption{Arquitectura software de supervisión en Raspberry Pi. La información publicada en la nube se consulta, valida, almacena en SQLite, expone mediante FastAPI y se presenta en el \textit{dashboard} web.}
