...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.72\textwidth]{figs_png/estrategia_incremental_validacion.png}
    \caption{Estrategia incremental de validación aplicada en TierraViva.}
    \label{fig:estrategia_incremental_validacion}
\end{figure}

La validación no se interpreta como certificación industrial del sistema, sino como comprobación técnica del prototipo desarrollado. El objetivo es demostrar que la arquitectura propuesta funciona, que los bloques se comunican correctamente y que la actuación sobre el riego incorpora medidas básicas de seguridad.
...
\hline
P09 & API FastAPI & Los endpoints de consulta devolvieron datos coherentes con la base de datos local. & Superada \\
\hline
P10 & Dashboard web & La interfaz mostró lecturas, estado de conexión, gráficas, histórico y selección de estación. & Superada \\
\hline
...
\hline
P13--P15 & Seguridad funcional & Se comprobaron cooldown, rechazo de lecturas no válidas y comportamiento seguro ante pérdida temporal de comunicación. & Superadas \\
\hline
P16 & Integración completa & Se validó el flujo extremo a extremo: estación, ThingSpeak, Raspberry Pi, SQLite, API, dashboard y actuación local. & Superada \\
\hline
P17 & Prueba funcional en Agrolab Móstoles & Se valida el funcionamiento del prototipo en un entorno real de cultivo, documentando el montaje, la publicación de telemetría, la visualización y la actuación controlada mediante evidencia fotográfica y vídeo. & Superada \\
\hline
\end{tabular}
...
El resultado global de las pruebas es positivo: los bloques esenciales del sistema han sido implementados y validados de forma integrada. Las observaciones más relevantes no están asociadas a fallos funcionales del prototipo, sino a limitaciones propias de esta primera versión: necesidad de una calibración agronómica más precisa del sensor de humedad de suelo, dependencia de cobertura LTE, consumo del módulo celular y validación prolongada en entorno real durante más tiempo.
...
\begin{figure}[H]
    ...
    \caption{Montaje físico utilizado para la validación de la estación remota (!!!!!cambiar por la definitiva!!!!!).}
...
Finalmente, se ejecutó el firmware conjunto de estación. Esta prueba confirmó que el Arduino podía leer humedad de suelo, temperatura, humedad relativa, presión y luminosidad en un mismo ciclo, actualizar su estado interno y dejar preparada una muestra completa de telemetría para su publicación.

La actuación se validó de forma progresiva: primero mediante relé e indicador visual, y después con la bomba de agua de 12 V como carga real. La prueba confirmó que el firmware activa el relé durante un intervalo limitado y lo apaga explícitamente al finalizar.

\section{Validación de comunicación, ThingSpeak y supervisión}
\label{sec:validacion_comunicacion_supervision}

La comunicación remota se validó primero de forma independiente y después integrada con Arduino. El A7670E-LASA respondió correctamente a comandos AT desde PC, reconoció la SIM, obtuvo señal, registró red LTE y permitió ejecutar peticiones HTTP. Posteriormente, el módem se conectó al Arduino mediante \texttt{SoftwareSerial}, trabajando a \texttt{9600} baudios para priorizar estabilidad.
...
La Raspberry Pi se validó consultando los canales ThingSpeak, evitando duplicados mediante \texttt{entry\_id}, registrando lecturas en SQLite y exponiendo los datos mediante FastAPI. El dashboard confirmó la visualización final de las lecturas, el estado de conexión, la antigüedad del dato, gráficas históricas y selección de estación.

La Figura~\ref{fig:evidencias_validacion_tierraviva} recoge evidencias visuales del flujo completo de validación.
...
\begin{minipage}{0.48\textwidth}
    \centering
    %\includegraphics[width=\textwidth]{figs/evidencia_sensores_monitor_serie.png}
    \small (a) Lectura conjunta de sensores.
\end{minipage}
\hfill
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figs_png/evidencia_thingspeak_publicacion.png}
    \small (b) Publicación de telemetría en ThingSpeak.
\end{minipage}
...
\begin{minipage}{0.48\textwidth}
    ...
    \small (c) Visualización en dashboard local.
\end{minipage}
\hfill
\begin{minipage}{0.48\textwidth}
    \centering
    %\includegraphics[width=\textwidth]{figs/evidencia_actuacion_bomba.png}
    \small (d) Prueba de actuación con relé y bomba.
\end{minipage}

\caption{Evidencias visuales de validación del flujo completo de TierraViva.}
...
Como cierre de la validación, se realizó una prueba funcional en Agrolab Móstoles, un entorno de agricultura urbana/periurbana situado en Móstoles, Madrid. Esta prueba no se plantea como una validación agronómica prolongada, sino como una comprobación funcional del prototipo fuera del entorno de laboratorio.

Durante la prueba se verificó el funcionamiento integrado de la estación de campo, la lectura de sensores, la comunicación LTE, la publicación de telemetría en ThingSpeak, la supervisión desde Raspberry Pi, la visualización en el dashboard y la actuación controlada del riego. La prueba permitió comprobar que el sistema mantiene el mismo flujo funcional en un entorno real de cultivo: medir, decidir localmente, actuar de forma temporizada, publicar estado y permitir la supervisión posterior.
...
La evidencia principal de esta prueba queda formada por la fotografía del montaje en Agrolab, las capturas de ThingSpeak y dashboard, y el vídeo de funcionamiento. En la redacción se mantiene la denominación de prueba funcional en entorno real, ya que la duración del ensayo no permite considerarla una campaña agronómica completa ni una validación estacional prolongada.

\section{Validación de actuación y seguridad funcional}
\label{sec:validacion_actuacion_seguridad}

La validación de la actuación se trató como una parte crítica del proyecto, ya que TierraViva acciona físicamente una bomba de agua. Por ello, no bastaba con comprobar que el relé podía activarse: también era necesario verificar que el firmware mantenía la bomba apagada ante condiciones no fiables.

Las pruebas se centraron en seis situaciones: arranque de estación, riego autorizado, cooldown activo, lectura inválida, pérdida temporal de comunicación y fallo de supervisión en Raspberry Pi. En todos los casos se verificó que la decisión crítica permanecía en la estación y que la bomba no dependía de una orden externa para apagarse.
...
\hline
Cooldown activo & La estación bloquea una nueva actuación antes del tiempo mínimo. & Correcto \\
\hline
Lectura inválida & El firmware no activa la bomba y publica estado de error. & Correcto \\
\hline
Pérdida temporal de comunicación & La estación mantiene control local y no genera actuación insegura. & Correcto \\
\hline
...
Estas pruebas demuestran que el prototipo incorpora una lógica de actuación prudente. TierraViva no ejecuta el riego por una decisión externa procedente de la nube, sino por decisión local de la estación. Además, la actuación está limitada en el tiempo y protegida por cooldown, validación de sensores y estado seguro por defecto. El análisis completo de riesgos asociados se recoge en el Anexo~\ref{anexo:gestion_riesgos}.

\section{Conclusión de la validación}
\label{sec:conclusion_validacion}

La validación realizada permite afirmar que TierraViva cumple el objetivo de integrar una estación IoT autónoma con sensórica ambiental, comunicación LTE, publicación en ThingSpeak, supervisión en Raspberry Pi, almacenamiento local, API, dashboard y actuación sobre riego. El sistema ha sido probado desde componentes individuales hasta el flujo completo extremo a extremo.

Las pruebas confirman la lectura conjunta de humedad del suelo, temperatura, humedad relativa, presión y luminosidad; el uso del A7670E-LASA mediante comandos AT, registro LTE y publicación HTTP; la consulta desde Raspberry Pi; el almacenamiento en SQLite; la exposición mediante API; la visualización en dashboard; la actuación local sobre relé y bomba; y la prueba funcional del prototipo en Agrolab Móstoles como entorno real de cultivo. sobre relé y bomba con temporización, cooldown y apagado seguro.
...
En conjunto, el resultado de la validación es satisfactorio para el alcance del Trabajo de Fin de Grado. El prototipo no se presenta como producto industrial terminado, sino como una arquitectura funcional, modular y verificable que demuestra el ciclo completo de monitorización y riego inteligente.
