...
\begin{minipage}{0.48\textwidth}
    ...
    \small (c) Visualización en \textit{dashboard} local.
\end{minipage}
\hfill
\begin{minipage}{0.48\textwidth}
    \centering
...
La evidencia principal de esta prueba queda formada por la fotografía del montaje en Agrolab, las capturas de ThingSpeak y \textit{dashboard}, y el vídeo de funcionamiento. En la redacción se mantiene la denominación de prueba funcional en entorno real, ya que la duración del ensayo no permite considerarla una campaña agronómica completa ni una validación estacional prolongada.

\section{Validación de actuación y seguridad funcional}
\label{sec:validacion_actuacion_seguridad}

La validación de la actuación se trató como una parte crítica del proyecto, ya que TierraViva acciona físicamente una bomba de agua. Por ello, no bastaba con comprobar que el relé podía activarse: también era necesario verificar que el \textit{firmware} mantenía la bomba apagada ante condiciones no fiables.

Las pruebas se centraron en seis situaciones: arranque de estación, riego autorizado, \textit{cooldown} activo, lectura inválida, pérdida temporal de comunicación y fallo de supervisión en Raspberry Pi. En todos los casos se verificó que la decisión crítica permanecía en la estación y que la bomba no dependía de una orden externa para apagarse.
...
\hline
Lectura inválida & El \textit{firmware} no activa la bomba y publica estado de error. & Correcto \\
\hline
...
Estas pruebas demuestran que el prototipo incorpora una lógica de actuación prudente. TierraViva no ejecuta el riego por una decisión externa procedente de la nube, sino por decisión local de la estación. Además, la actuación está limitada en el tiempo y protegida por \textit{cooldown}, validación de sensores y estado seguro por defecto. El análisis completo de riesgos asociados se recoge en el Anexo~\ref{anexo:gestion_riesgos}.
