...
MQTT utiliza un modelo de publicación/suscripción. Los dispositivos publican mensajes en un intermediario llamado \textit{broker}, y los clientes interesados se suscriben a determinados \textit{topics}. MQTT está estandarizado por OASIS (\textit{Organization for the Advancement of Structured Information Standards}) como un protocolo ligero de mensajería publicación/suscripción para contextos IoT y M2M (\textit{Machine to Machine}) \cite{oasis_mqtt50}. Este modelo desacopla productores y consumidores, aunque requiere un \textit{broker} disponible, configurado y protegido.
