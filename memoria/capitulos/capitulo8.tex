...
Tras la implementación y validación de TierraViva, este capítulo sintetiza los resultados obtenidos y discute el alcance real del sistema desarrollado. La finalidad no es repetir la implementación descrita en los capítulos anteriores, sino valorar el prototipo como una solución completa: qué se ha conseguido, qué decisiones han sido más relevantes, cómo se ha comportado en pruebas reales y qué mejoras permitirían evolucionarlo hacia una versión más robusta.

TierraViva no se limita a una prueba aislada de sensores ni a una visualización de datos. El resultado final es un sistema IoT funcional formado por dos estaciones remotas, comunicación LTE, publicación en ThingSpeak, supervisión local en Raspberry Pi, almacenamiento en SQLite, API FastAPI, \textit{dashboard} web y actuación física sobre riego. Esta integración es el aspecto central del trabajo: el sistema mide, decide, actúa, publica, almacena y visualiza.
...
Cada estación ejecuta localmente el \textit{firmware} de adquisición, decisión y actuación. La Raspberry Pi no controla la bomba ni envía órdenes de riego, sino que actúa como nodo de supervisión: consulta ThingSpeak, almacena lecturas en SQLite, expone datos mediante API y sirve el \textit{dashboard}. Esta separación permite que la actuación crítica permanezca en la estación incluso si la supervisión o la comunicación fallan temporalmente.
...
\hline
Estaciones remotas & Dos estaciones físicas implementadas, configuradas y probadas en entorno de laboratorio y Agrolab Móstoles. & Montaje físico, vídeo, monitor serie y prueba funcional en entorno real. \\
\hline
...
\hline
Validación real & Prueba funcional con dos estaciones en Agrolab Móstoles, en zonas con diferente humedad de suelo. & Fotografías, vídeo, capturas, monitor serie y observación directa. \\
\hline
\end{tabular}
...
\renewcommand{\arraystretch}{1.18}
\rowcolors{2}{TableRowSoft}{white}
\begin{tabular}{|p{0.19\textwidth}|p{0.32\textwidth}|p{0.26\textwidth}|p{0.13\textwidth}|}
\hline
\rowcolor{URJCRed}
...
\hline
Decisión local de riego & El riego se decide en el \textit{firmware}, no desde la nube ni desde la Raspberry Pi. & RF-08, RF-11, RNF-06; P13--P15. & Cumplido \\
\hline
...
\hline
Validación en entorno real & El prototipo se prueba en Agrolab Móstoles con dos estaciones y diferentes condiciones de humedad. & P17; validación funcional en entorno real. & Cumplido \\
\hline
\end{tabular}
...
La arquitectura final de TierraViva es el resultado de varias decisiones tomadas durante el desarrollo. La más importante ha sido mantener la actuación crítica en la estación. Esta decisión condiciona todo el sistema: la nube no ordena el riego, la Raspberry Pi no activa la bomba y el \textit{dashboard} no fuerza actuaciones. La estación mide y decide localmente, mientras que el resto de bloques supervisa, almacena y visualiza.

Esta separación resulta especialmente adecuada en un sistema físico que actúa sobre agua. Si la decisión dependiera de la nube o de una conexión continua, una caída de red, una latencia elevada o una orden incorrecta podrían provocar un comportamiento no deseado. En TierraViva, una pérdida temporal de ThingSpeak afecta a la publicación de datos, pero no deja la bomba sin control.
...
ThingSpeak ha funcionado como una capa intermedia útil para telemetría. Su valor principal ha sido desacoplar las estaciones de la Raspberry Pi: la estación publica desde campo y la Raspberry consulta posteriormente. Sin embargo, ThingSpeak no se interpreta como núcleo de control ni como sistema de tiempo real. Su papel en TierraViva es recibir datos, conservarlos temporalmente y permitir su consulta.
...
La existencia de dos estaciones montadas y probadas refuerza la validez de la arquitectura. Aunque TierraViva no se presenta como red agrícola de gran escala, sí demuestra que el modelo no queda restringido a un único nodo. La separación por identificadores y canales permite escalar conceptualmente a más estaciones, manteniendo una lógica común de supervisión.
...
Durante la prueba se desplegaron las dos estaciones en distintas zonas de cultivo, con condiciones de humedad diferentes. Esto permitió comprobar si la lectura del sensor era coherente con el estado del sustrato y si la lógica de riego respondía de forma razonable ante zonas más secas o más húmedas. La prueba no constituye una campaña agronómica prolongada, pero sí una validación funcional relevante en un contexto de uso real.

Además de la comprobación técnica, la prueba permitió recoger impresiones espontáneas de personas usuarias del espacio. Estas observaciones no se presentan como encuesta formal ni como estudio estadístico, pero sí como una primera recepción cualitativa del prototipo por parte de personas familiarizadas con el riego, el cuidado de cultivos y las dificultades prácticas de mantener un huerto.
...
Esta recepción cualitativa refuerza una conclusión importante: TierraViva no solo resuelve un problema técnico de integración IoT, sino que conecta con una necesidad comprensible para usuarios reales de huertos urbanos y pequeñas zonas de cultivo. Esa conexión entre implementación técnica y utilidad percibida es uno de los resultados más relevantes de la prueba en Agrolab.
...
Por último, la prueba en Agrolab Móstoles es una validación funcional en entorno real, no una certificación agronómica. No permite afirmar ahorros hídricos cuantificados, mejoras de rendimiento del cultivo ni comportamiento estacional. Estas conclusiones requerirían campañas de larga duración y mediciones comparativas.
...
La segunda línea futura es la autonomía energética. La integración de panel solar, batería dimensionada y regulador de carga permitiría desplegar estaciones durante periodos más largos. Esta mejora fue especialmente coherente con los comentarios recibidos en Agrolab, donde se valoró la posibilidad de adaptar batería, alimentación y depósito a cada caso.

Una tercera línea es incorporar configuración remota desde el \textit{dashboard}. Esta mejora permitiría modificar umbrales de humedad, duración de riego, frecuencia de envío o parámetros de \textit{cooldown} sin reprogramar el Arduino. No obstante, no debe plantearse como un simple botón añadido a la interfaz: implicaría diseñar un canal descendente seguro, validación de órdenes, persistencia de configuración y límites que impidan actuaciones peligrosas.
...
Desde el punto de vista físico, una evolución natural sería diseñar una PCB propia, mejorar el encapsulado, emplear conectores estancos, ordenar el cableado y preparar una caja de campo. Estas mejoras no cambian la arquitectura conceptual, pero sí aumentarían la robustez y facilidad de instalación.
...
La alineación de TierraViva con los Objetivos de Desarrollo Sostenible se entiende como una contribución potencial dentro del alcance de un prototipo académico. El sistema no cuantifica todavía ahorro hídrico ni demuestra una mejora agronómica prolongada, pero sí materializa una arquitectura orientada a medir condiciones reales del entorno, evitar actuaciones de riego puramente fijas, supervisar el estado del sistema y facilitar futuras optimizaciones basadas en datos.
...
Desde el punto de vista social, la prueba en Agrolab Móstoles mostró que el sistema puede resultar comprensible y útil para usuarios no técnicos, especialmente por la posibilidad de supervisar el estado del riego, automatizar actuaciones durante ausencias y adaptar el montaje a distintas necesidades. Esta recepción refuerza que el valor del prototipo no reside solo en la integración tecnológica, sino también en su aplicabilidad en huertos urbanos y pequeñas zonas de cultivo.
