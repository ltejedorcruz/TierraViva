...
Esta primera versión permitió comprobar que una arquitectura por capas facilitaba la depuración. Cada bloque tenía una responsabilidad concreta: Arduino generaba lecturas, el bridge serie--MQTT adaptaba la entrada, Mosquitto actuaba como intermediario de mensajes, el backend procesaba y almacenaba los datos, SQLite conservaba el histórico, FastAPI exponía la información y el dashboard permitía visualizar el estado del sistema.
...
Sin embargo, esta solución presentaba limitaciones importantes para el objetivo final de TierraViva. La dependencia de una conexión local entre Arduino y Raspberry Pi no era adecuada para estaciones situadas en una parcela o zona alejada. Además, la lógica provisional de actuación desde el backend desplazaba parte de la decisión hacia la Raspberry Pi, lo que introducía una dependencia no deseada en un sistema que debía mantener capacidad de actuación local y segura.

Por estos motivos, el prototipo MQTT no se adoptó como arquitectura definitiva. La solución final sustituye la comunicación local Serial--MQTT por publicación remota LTE/HTTP en ThingSpeak. Asimismo, la decisión de riego pasa al firmware de cada estación, mientras que la Raspberry Pi queda como nodo de supervisión, ingesta, almacenamiento, API y dashboard.
...
Esta fase previa no se considera un desvío del proyecto, sino una etapa útil de validación incremental. Permitió comprobar en pequeño una cadena software completa y conservar sus partes más sólidas en la versión final: ingesta de datos, almacenamiento local, API y dashboard.
...
Por ejemplo, una estación puede publicar un evento de riego activado, de riego finalizado, de error o de cooldown. El diagnóstico permite complementar ese evento con un código de funcionamiento correcto, no riego, modo seguro, error o bloqueo por cooldown. Esta separación facilita que el dashboard no muestre únicamente si hay riego o no, sino una explicación mínima del motivo.
...
\hline
API FastAPI & Devuelve lecturas y estado al dashboard mediante endpoints HTTP. \\
\hline
Dashboard & Traduce los estados a información visual comprensible para el usuario. \\
\hline
...
\hline
\texttt{/} & Sirve el dashboard web al navegador. \\
\hline
...
\hline
\texttt{/api/health} & Comprueba el estado básico de API, base de datos y frontend. \\
\hline
\end{tabular}
...
Uno de los aspectos más relevantes de la API es que no se limita a devolver registros crudos de la base de datos. Algunas respuestas incorporan información calculada que simplifica el trabajo del dashboard. Por ejemplo, a partir de la última lectura puede calcularse la antigüedad del dato, una etiqueta de frescura y una clasificación básica de humedad del suelo. De este modo, la interfaz no necesita repetir lógica de interpretación en JavaScript.

La API también ayuda a separar responsabilidades en la visualización. El dashboard solicita datos ya estructurados, mientras que la API concentra las reglas de consulta y preparación de respuesta. Esta decisión reduce acoplamiento entre frontend y base de datos, y facilita la depuración: si una lectura aparece incorrecta en la interfaz, puede comprobarse primero la respuesta JSON de la API antes de revisar la capa visual.

La referencia completa de endpoints, parámetros, ejemplos de respuesta, validaciones y códigos HTTP se incluye en el Anexo~\ref{anexo:manual_api}. El uso de estos endpoints desde el punto de vista del usuario final se describe en el Anexo~\ref{anexo:manual_usuario}.

En resumen, FastAPI permite convertir la información almacenada localmente en una interfaz de consulta clara y reutilizable. Dentro de TierraViva no cumple una función de control físico, sino de supervisión: sirve datos al dashboard, facilita la consulta del histórico y permite comprobar el estado básico del sistema software.

\section{Dashboard web de supervisión}
\label{sec:dashboard_web_supervision}

El dashboard web constituye la capa de presentación del sistema software de TierraViva. Su función es transformar los datos servidos por la API en una vista de supervisión accesible desde un navegador, sin que el usuario tenga que consultar directamente ThingSpeak, SQLite o los logs de ejecución.

Dentro de la arquitectura final, el dashboard no toma decisiones de riego, no accede al puerto serie, no escribe en la base de datos y no envía órdenes a las estaciones. Su papel se limita a consultar la API local, representar el estado del sistema y actualizar la información mostrada conforme llegan nuevos datos.

La implementación del frontend se organiza principalmente en dos bloques:
...
\subsection{Papel del dashboard dentro de la arquitectura}

El dashboard se sitúa al final del flujo software de TierraViva. Las estaciones publican telemetría en ThingSpeak, la Raspberry Pi ingiere y almacena los datos en SQLite, FastAPI los expone mediante endpoints HTTP y el dashboard los presenta visualmente.

\begin{center}
\texttt{ThingSpeak} $\rightarrow$ \texttt{SQLite} $\rightarrow$ \texttt{FastAPI} $\rightarrow$ \texttt{Dashboard}
\end{center}

Esta organización evita que la interfaz web dependa directamente de la plataforma cloud o de la estructura interna de la base de datos. Si se modifica el almacenamiento local o el mecanismo de ingesta, el dashboard puede mantenerse estable siempre que la API conserve el mismo contrato de respuesta.

El flujo completo de responsabilidades entre estación, plataforma IoT, Raspberry Pi, API y dashboard se resume en el Anexo~\ref{anexo:diagramas_flujo_funcionamiento}. El uso de la interfaz desde el punto de vista del usuario final se documenta en el Anexo~\ref{anexo:manual_usuario}.

\subsection{Consumo de datos desde la API}

El dashboard consume únicamente los datos expuestos por FastAPI. En particular, utiliza endpoints de configuración, última lectura, histórico reciente y estadísticas agregadas. No se detallan aquí los parámetros ni las respuestas de cada endpoint, ya que la referencia completa se recoge en el Anexo~\ref{anexo:manual_api}.

Desde el punto de vista del frontend, las consultas cumplen tres funciones principales:
...
La estación activa se gestiona mediante un selector en la interfaz. Al cambiar de estación, el JavaScript actualiza las consultas realizadas a la API y reconstruye los elementos visuales con los datos correspondientes. Esto permite reutilizar la misma interfaz para varias estaciones sin duplicar páginas ni lógica de frontend.

\subsection{Actualización periódica de la interfaz}

La actualización de la interfaz se realiza mediante peticiones HTTP periódicas desde JavaScript. El intervalo de refresco se obtiene de la configuración devuelta por la API, de forma que el comportamiento del dashboard pueda ajustarse desde la configuración del sistema sin modificar el código visual.

El dashboard no asume que cada consulta vaya a producir una lectura nueva. La estación publica datos con su propio intervalo y la Raspberry Pi puede consultar ThingSpeak con una frecuencia distinta. Por ello, el frontend trabaja con la última información servida por la API y utiliza los campos de frescura para distinguir entre datos recientes, antiguos u offline.
...
\hline
\rowcolor{gray!20}
\textbf{Bloque del dashboard} & \textbf{Función técnica} \\
\hline
...
\hline
\end{tabular}
\caption{Bloques funcionales del dashboard web.}
...
Una decisión relevante del diseño es que el dashboard no cierre el bucle de control del riego. Aunque muestra estados operativos como relé activo, evento de sistema o código de diagnóstico, estos valores proceden de la estación y han sido publicados previamente en ThingSpeak. La interfaz no modifica umbrales en Arduino, no activa la bomba y no confirma órdenes de actuación.

Esta separación reduce el riesgo de que un fallo de navegador, una pérdida de conexión local o un error de JavaScript afecten al comportamiento físico de la estación. Si el dashboard deja de estar disponible, la estación puede seguir ejecutando su lógica local de medida, decisión y actuación.

\subsection{Gestión visual de errores y datos ausentes}

El dashboard debe representar también situaciones incompletas o anómalas. Si la API no devuelve datos para una estación, si una lectura es antigua o si algún campo aparece vacío, la interfaz debe reflejar esa condición en lugar de ocultarla. En un sistema de supervisión, la ausencia de datos también es información relevante.

Este comportamiento es especialmente importante en TierraViva porque el sistema trabaja con telemetría periódica. La última lectura almacenada no siempre equivale al estado físico instantáneo de la estación. Por ello, el dashboard combina valores medidos, marca temporal, frescura de datos y códigos de estado para ofrecer una vista más prudente del sistema.

\subsection{Conclusión}

El dashboard web completa la arquitectura software de TierraViva como capa de presentación. Su valor no reside en controlar directamente el riego, sino en hacer observable el sistema: muestra lecturas, evolución temporal, estado operativo y trazabilidad reciente a partir de los datos servidos por la API.

Con esta organización se mantiene una separación clara de responsabilidades: Arduino mide, decide y actúa; ThingSpeak recibe la telemetría; Raspberry Pi ingiere y almacena; FastAPI expone los datos; y el dashboard los presenta de forma accesible. El uso detallado de la interfaz se documenta en el Anexo~\ref{anexo:manual_usuario}.
...
El script permite instalar dependencias, preparar el entorno virtual, preparar el directorio del dashboard servido por FastAPI, arrancar el proceso de ingesta \texttt{main.py}, iniciar el servidor \texttt{api.py} mediante Uvicorn, consultar el estado de ejecución, detener los procesos y revisar los logs. Esta automatización resulta útil durante las pruebas porque reduce errores repetitivos de arranque y deja una forma reproducible de iniciar el sistema.
