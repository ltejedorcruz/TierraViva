...
Una vez identificado el conversor \texttt{CH340G}, se instaló el driver correspondiente y Windows pasó a reconocer la placa como un dispositivo serie asociado al puerto \texttt{COM3}. A partir de ese momento, el Arduino IDE permitió seleccionar la placa como \textit{Arduino Uno} y cargar programas correctamente.
...
\hline
\rowcolor{URJCRed}
\TVHead{Bloque del firmware} & \TVHead{Función principal} \\
\hline
...
Para evitar que el cuerpo principal de la memoria se convierta en un listado completo de firmware, la Tabla~\ref{tab:responsabilidades_firmware_estacion} resume las responsabilidades principales implementadas en el código de estación. El código completo se mantiene en el repositorio del proyecto.
...
\hline
\rowcolor{URJCRed}
\TVHead{Función del firmware} & \TVHead{Responsabilidad dentro de la estación} \\
\hline
...
\hline
Cooldown & Bloquea riegos consecutivos durante un periodo mínimo tras una actuación. \\
\hline
Publicación de estado & Actualiza el estado del relé, el evento de sistema y el código auxiliar de diagnóstico. \\
\hline
\end{tabular}
\caption{Responsabilidades principales del firmware de estación.}
...
Las dependencias principales del firmware, incluyendo las librerías utilizadas para comunicación serie, bus I2C, lectura de sensores y manejo de cadenas, se resumen en el Anexo~\ref{anexo:manual_instalacion_sistema}. Esta información se documenta como parte de la reproducibilidad del montaje, ya que resulta necesaria para compilar y cargar correctamente el sketch de estación.
...
La integración funcional del módem se organizó en las fases resumidas en la Tabla~\ref{tab:fases_integracion_a7670e}. El detalle completo de comandos AT no se incluye en el cuerpo principal para evitar convertir el capítulo en una referencia de firmware; se conserva en el código fuente del repositorio.
...
\hline
Robustez & Si falla la red, el firmware no bloquea indefinidamente el ciclo principal. \\
\hline
\end{tabular}
...
El envío se realiza mediante una petición HTTP de actualización de canal. ThingSpeak permite escribir datos en un canal mediante una URL que incluye la clave de escritura y los campos que se desean actualizar \cite{thingspeak_write_data}. En el caso de TierraViva, el firmware construye esa petición a partir de las variables medidas por los sensores y de varios campos internos de estado.

La correspondencia entre variables de firmware y campos de ThingSpeak se mantiene fija para que la Raspberry Pi pueda interpretar todas las estaciones de forma homogénea. Los campos \texttt{field1}--\texttt{field5} transportan variables físicas: humedad de suelo, temperatura, humedad relativa, luminosidad y presión. Los campos \texttt{field6}, \texttt{field7} y \texttt{field8} publican información operativa: estado del relé, evento de sistema y código auxiliar de diagnóstico.

La estructura completa de campos, nombres internos y códigos se documenta en el Anexo~\ref{anexo:estructura_thingspeak_estados}. En este capítulo se conserva únicamente la idea de implementación: el firmware construye una petición HTTP de actualización de canal, la envía mediante el A7670E-LASA y considera correcta la publicación cuando la respuesta del módem indica código HTTP 200.

La gestión de respuesta es importante porque el hecho de enviar una petición al módem no garantiza por sí solo que ThingSpeak haya registrado una nueva entrada. Por ello, el firmware comprueba la respuesta del comando HTTP correspondiente. Si la respuesta incluye un código HTTP 200, la publicación se considera correcta; en caso contrario, la estación informa del fallo por monitor serie y continúa con el siguiente ciclo.
...
\hline
Actuación temporizada & El firmware no depende de una orden externa de parada. \\
\hline
Cooldown entre riegos & Evita activaciones repetidas por ruido o lecturas inestables. \\
\hline
...
Esta lógica tiene una ventaja importante para el alcance actual del prototipo: la activación y el apagado quedan controlados por el propio firmware de la estación. La bomba no queda a la espera de una orden externa de parada ni depende de la Raspberry Pi. El propio firmware activa el relé, espera el tiempo configurado y lo apaga de forma explícita, como se mostró en el Código~\ref{cod:activacion_temporizada_rele}.
