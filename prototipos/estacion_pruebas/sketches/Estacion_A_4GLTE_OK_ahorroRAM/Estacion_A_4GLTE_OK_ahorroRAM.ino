...
const unsigned long NETWORK_RETRY_MS = 10000UL;
...
bool sendThingSpeak(int soilPercent, float tempC, float humPct, int lux, int pressureHpa) {
  ...
  // Temperatura y humedad con 1 decimal
  ...
  readReply(resp, sizeof(resp), 12000);
...
void setup() {
  ...
  Serial.println(F("=== TierraViva RAM ahorro - todos los sensores ==="));
...
void loop() {
  ...
  // Presión en entero para ahorrar RAM
